...
class JobApplicationRepository {
  ...
  JobApplicationRepository({HybridDatabaseService? db}) : _db = db ?? hybridDatabase;

  /// Create a new job application
  Future<void> create(JobApplication application) async {
    await _db.insertJobApplication(_toMap(application));
  }
  ...
  Future<JobApplication?> getById(String id) async {
    final rows = await _db.rawQuery('SELECT * FROM job_applications WHERE id = ?', positional: [id]);
    if (rows.isEmpty) return null;
    return _fromMap(rows.first);
  }

  /// Update application status
  Future<void> updateStatus(String id, ApplicationStatus status, {String? rejectionReason}) async {
    await _db.updateJobApplicationStatus(id, status.name, rejectionReason: rejectionReason);
  }
  ...
  Future<void> addInterviewDate(String id, DateTime interviewDate) async {
    ...
    final dates = List<DateTime>.from(app.interviewDates ?? [])..add(interviewDate);
    dates.sort();
  ...
  Future<List<JobApplication>> getByStatus(ApplicationStatus status) async {
    final rows = await _db.rawQuery('SELECT * FROM job_applications WHERE status = ? ORDER BY applied_at DESC',
        positional: [status.name]);
    return rows.map(_fromMap).toList();
  }

  /// Get recent applications (last N days)
  Future<List<JobApplication>> getRecent({int days = 30, String? candidateId}) async {
  ...
  String _encodeDates(List<DateTime>? dates) {
    ...
    return '[${dates.map((d) => d.millisecondsSinceEpoch).join(',')}]';
  }

  List<DateTime>? _decodeDates(String? encoded) {
    if (encoded == null || encoded.isEmpty || encoded == '[]') return null;
    try {
      final timestamps = (encoded.replaceAll('[', '').replaceAll(']', '').split(','))
